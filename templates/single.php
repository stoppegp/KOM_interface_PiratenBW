<?php
KOM::registerScript('function focusLink(isOnFocus, id)
{
    if (isOnFocus)
            ...
            if (is_array($issue->getPledgesOfParty($pid))) {
                foreach ($issue->getPledgesOfParty($pid) as $pledge) {
                    ...
                    <div class="pledge_quote">
                        ...
                        ?>«</span>
                        <span class="pledge_quote_source">(<a onFocus="focusLink(true, 'switch-action-p<?=$pledge->getID();?>')" onBlur="focusLink(false, 'switch-action-p<?=$pledge->getID();?>')" href="<?php echo $pledge->getQuoteURL(); ?>"><?php
                    ...
                    (<a onFocus="focusLink(true, 'switch-action-p<?=$pledge->getID();?>')" onBlur="focusLink(false, 'switch-action-p<?=$pledge->getID();?>')" href="<?=KOM::dolink("single", array("issueid" => $issue->getID()));?>#state_<?=$pledge->getCurrentState()->getID();?>"><?=date("d.m.Y", $pledge->getCurrentState()->getDatum());?></a>)
                    <? } ?>
                    </div>
                    </div></div>
                    <?php
...
foreach ($issue->getStates("datum", "DESC") as $state) {
    ...
    <div class="switch-action state-infos" id="switch-action-s<?=$state->getID();?>">
                    <div class="state_quote">
                        ...
                        ?>«</span>
                        <span class="state_quote_source">(<a onFocus="focusLink(true, 'switch-action-s<?=$state->getID();?>')" onBlur="focusLink(false, 'switch-action-s<?=$state->getID();?>')" href="<?php echo $state->getQuoteURL(); ?>"><?php
